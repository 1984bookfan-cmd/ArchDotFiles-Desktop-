    ...
    for i, day in enumerate(weather['weather']):
        ...
        for hour in day['hourly']:
            if i == 0 and int(format_time(hour['time'])) < datetime.now().hour - 2:
                continue
            data['tooltip'] += f"{format_time(hour['time'])} {WEATHER_CODES.get(hour['weatherCode'],'?')} {format_temp(hour['FeelsLikeF'])} {hour['weatherDesc'][0]['value']}, {format_chances(hour)}\n"

    print(json.dumps(data))

except Exception:
    # Fallback: blank space, keeps Waybar showing the module box
    print(json.dumps({"text": " ", "tooltip": "No data"}))
